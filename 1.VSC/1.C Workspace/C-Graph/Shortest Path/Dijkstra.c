    ...
    previous[s] = -1;
     ...
     for(size_t i = 1; i <= g->max_vertex; i++){
        ...
        for(size_t vertex = 1; vertex <= g->max_vertex; vertex++){
            ...
            if( g->weight[ chosen_vertex ][ vertex ] != 0 && !visited[ vertex ]){
...
/* Bug history:
    ...
    - Vòng lặp for in kết quả vô tình nằm trong vòng lặp điều chỉnh min_path ( vòng lặp thứ 2) gây ra lỗi lặp kết quả khi in.
*/ 
